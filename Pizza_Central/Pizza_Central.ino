...
static void sendSoundPlay(uint8_t houseId, uint8_t clipId, uint8_t vol){
  SoundPlayPayload p{}; p.house_id = houseId; p.clip_id = clipId; p.vol = vol;
  uint8_t buf[64];
...
void loop() {
  ...
  if (line.length()) {
    ...
    if (line == "hello-req") { sendHelloReq(); }
      ...
      uint8_t id   = (uint8_t)parseInt(line.substring(s1+1, s2), 1);
      uint8_t clip = (uint8_t)parseInt(line.substring(s2+1, s3>0? s3: line.length()), 1);
      uint8_t vol  = 200;
      if (s3>0) vol = (uint8_t)parseInt(line.substring(s3+1), 200);

      sendSoundPlay(id, clip, vol);
    }
  }
}
